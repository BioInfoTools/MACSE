...
\begin{figure}[H]
	\center{\includegraphics[width=0.95\linewidth]{block-scheme.png}}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\If{$size(F)$ $<$ $len(P_1)$ OR $size(F)$ $<$ $len(P_2)$} \Comment{Проверка размерности матрицы}
		...
		\For{$i = 1 \cdots len(P_1)$} \Comment{Цикл заполнения таблицы переходов}
			\For{$j = 1 \cdots len(P_2)$}
		        \State \text{Рассчет оптимального перехода,} \Comment{Позиция текущей ячейки --- $i$, $j$}
